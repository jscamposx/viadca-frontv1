...
const METHOD_ICONS: Record<string, LucideIcon> = { MessageCircle, Phone, Mail };
          ...
          return (
            ...
            >
              ...
              <div className="relative z-10 flex flex-col flex-1">
                ...
                <address className="font-mono text-foreground font-medium text-sm mb-6 not-italic">{method.value}</address>
          ...
          <div className="absolute top-6 left-6 z-20 max-w-[calc(100%-3rem)]">
            <div className="flex items-center gap-4 p-4 bg-background/95 backdrop-blur-md rounded-2xl border border-muted/20 shadow-sm">
              ...
              <address className="min-w-0 not-italic">
                ...
                <p className="text-muted text-sm mt-0.5 font-medium leading-snug wrap-break-word">{CONTACT_LOCATION.address}</p>
              </address>
            </div>
          </div>
          <a
          ...
          <section className="bg-surface rounded-3xl p-8 shadow-sm border border-muted/10 w-full">
            ...
            <ul className="space-y-4">
              {CONTACT_HOURS.map((item) => {
                ...
                return (
                  <li key={item.dia} className="flex flex-wrap justify-between items-center p-4 rounded-xl bg-background border border-muted/10 gap-3">
                    <div className="flex items-center gap-3">
                      <Calendar className="w-4 h-4 text-muted shrink-0" />
                      <span className="text-foreground font-medium">{item.dia}</span>
                    </div>
                    <time className={`px-3 py-1.5 rounded-lg font-bold text-xs whitespace-nowrap ${isClosed ? "bg-muted/10 text-muted" : "bg-brand/10 text-brand"}`}>
                      {item.horario}
                    </time>
                  </li>
                );
                  ...
                  return (
                    <li key={social.name}>
                      ...
                      >
                        ...
                        <span className="text-xs font-bold text-muted group-hover:text-foreground transition-colors text-center">{social.name}</span>
                      </a>
                    </li>
                  );
                })}
